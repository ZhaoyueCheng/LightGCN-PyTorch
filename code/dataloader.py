...
class AmazonLoader(BasicDataset):
    ...
    #     # self.trainItem = np.array(trainItem)
    #
    #     train = np.load(train_file).astype(int)
    ...
    #     #             self.testDataSize += len(items)
    #
    #     test = np.load(test_file).astype(int)
...
class MlLoader(BasicDataset):
    ...
    def __init__(self, config=world.config, path="../data/ml/"):
        import pickle
        ...
        self.n_user = 0
        self.m_item = 0
        train_file = path + '/train_set.pkl'
        test_file = path + '/test_set.pkl'
        self.path = path
        trainUniqueUsers, trainItem, trainUser = [], [], []
        testUniqueUsers, testItem, testUser = [], [], []
        self.traindataSize = 0
        ...
        train = pickle.load(open(train_file, 'rb'))
        for uid, item_list in enumerate(train):
            trainUniqueUsers.append(uid)
            trainUser.extend([uid] * len(item_list))
            trainItem.extend(item_list)
            self.n_user = max(self.n_user, uid)
            self.m_item = max(self.m_item, max(item_list))
            self.traindataSize += len(item_list)
        ...
        test = pickle.load(open(test_file, 'rb'))
        for uid, item_list in enumerate(test):
            testUniqueUsers.append(uid)
            testUser.extend([uid] * len(item_list))
            testItem.extend(item_list)
            self.n_user = max(self.n_user, uid)
            self.m_item = max(self.m_item, max(item_list))
            self.testDataSize += len(item_list)
